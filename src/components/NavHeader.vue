<template>
	<header class="header">
       ...
       <div class="md-modal modal-msg md-modal-transition" :class="{'md-show':loginModalFlag}">
        ...
        <div class="md-modal modal-msg md-modal-transition" :class="{'md-show':registerModalFlag}">
          <div class="md-modal-inner">
            ...
            <div class="md-content">
              <div class="confirm-tips">
                <div class="error-wrap">
                  <span class="error error-show" v-show="reErrorTip">用户名或者密码不能为空</span>
                </div>
                <ul>
                  <li class="regi_form_input">
                    <i class="icon IconPeople"></i>
                    <input type="text" tabindex="1" name="loginname" v-model="reUserName" class="regi_login_input regi_login_input_left" placeholder="User Name" data-type="loginname">
                  </li>
                  <li class="regi_form_input noMargin">
                    <i class="icon IconPwd"></i>
                    <input type="password" tabindex="2"  name="password" v-model="reUserPwd"  class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="Password" @keyup.enter = "register">
                  </li>
                </ul>
              </div>
              <div class="login-wrap">
        ...
        <div class="md-overlay" v-show="registerModalFlag" @click="registerModalFlag=false"></div>
	</header>
</template>
    ...
    methods:{
      ...
      register(){
        if(!this.reUserName || !this.reUserPwd){
          this.reErrorTip = true;
          return;
        }
        axios.post('/users/register',{userName:this.reUserName,userPwd:this.reUserPwd}).then((response)=>{
          let res = response.data;
          if(res.status=='0'){
            this.reErrorTip = false;
